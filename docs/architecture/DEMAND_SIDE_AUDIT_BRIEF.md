...
| Art | `1b1c962af54c6dc5616b713a5f5f41ec017b324b` | The active release resolves to `/opt/aipg-gallery-releases/gallery-1b1c962a`; production-config frontend build, Go tests, `go vet`, binary build, public Studio/Director routes, removed audio routes, and unauthenticated job/credit rejection all passed. Every protected Grid call now fails closed unless Core's exchanged canonical `account_id` matches the signed Gallery session account. |
