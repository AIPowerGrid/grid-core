...
"""Preview or apply a validator operator-independence review."""
...
from grid_api.services.validator_operators import review_operator


async def _run(args) -> None:
    await init_database()
    try:
        result = await review_operator(
            args.validator_id,
            action=args.action,
...
def main() -> None:
    ...
    parser.add_argument("--action", choices=("candidate", "verify", "reject"), required=True)
